        ...
        autocmd({ "BufReadPost" }, {
                ...
                callback = function()
                        ...
                        local closestFsFile = vim.fs.find(function(name, path)
                            ...
                            end, { limit = 1, type = "file", upward = true, path = parentDir, stop = projectRoot })[1] or
                            (function()
                                    local newFile = parentDir ..
                                        "/" .. vim.inspect(os.time()) .. "TempFileForProjectInitDeleteMe.fs"
        ...
        vim.lsp.buf_request(0, "fsproj/renameFile", payload,
                function(payload)
                        if payload then
                                ...
                                util.notify("Renamed file " .. existingFile .. " to: " .. existingFile)
                        end
                end)
end

function M.RemoveCurrentFile()
        local bufnr = vim.api.nvim_get_current_buf()
        local bufname = vim.fs.normalize(vim.api.nvim_buf_get_name(bufnr))
        os.remove(bufname)
        local cwd = vim.fs.normalize(vim.fs.dirname(bufname))
        ...
        local fixedFile = existingFile:gsub(result.path .. "/", "")
        local msFile = fixedFile:gsub("/", "\\")
        M.RemoveFile(result.file, msFile)
end

function M.RemoveFileManually(proj, file)
        local bufnr = vim.api.nvim_get_current_buf()
        local bufname = vim.fs.normalize(vim.api.nvim_buf_get_name(bufnr))
        local cwd = vim.fs.normalize(vim.fs.dirname(bufname))
        local result = util.find_proj_ancestor(cwd)
        local msFile = fixedFile:gsub("\\", "/")
        os.remove(result.path .. msFile)
        M.RemoveFile(result.file, file)
end

function M.RemoveFile(proj, file)
        local payload = {
                FsProj = proj,
                FileVirtualPath = file,
        }
        vim.lsp.buf_request(0, "fsproj/removeFile", payload,
                function(payload)
                        if payload then
                                util.notify(vim.inspect(payload))
                        else
                                util.notify("Removed file " .. file .. " from project: " .. proj)
                        end
                end)
end

function M.AddNewFile(proj, file)
        local bufnr = vim.api.nvim_get_current_buf()
        local bufname = vim.fs.normalize(vim.api.nvim_buf_get_name(bufnr))
        local cwd = vim.fs.normalize(vim.fs.dirname(bufname))
        local result = util.find_proj_ancestor(cwd)
        local msFile = fixedFile:gsub("\\", "/")
        cFile = io.open(result.path .. msFile, "w")
        cFile:close()
        ...
        vim.lsp.buf_request(0, "fsproj/addFile", payload,
                function(payload)
                        if payload then
                                util.notify(vim.inspect(payload))
                        else
                                util.notify("Added file " .. file .. " to project: " .. proj)
                        end
                end)
end

function M.AddExistingFile(proj, file)
        local bufnr = vim.api.nvim_get_current_buf()
        local bufname = vim.fs.normalize(vim.api.nvim_buf_get_name(bufnr))
        local cwd = vim.fs.normalize(vim.fs.dirname(bufname))
        local result = util.find_proj_ancestor(cwd)
        ...
        vim.lsp.buf_request(0, "fsproj/addExistingFile", payload,
                function(payload)
                        if payload then
                                util.notify(vim.inspect(payload))
                        else
                                util.notify("Added existing file " .. file .. " to project: " .. proj)
                        end
                end)
end
...
function M.SetupFileMoveCommands()
        ...
        uc("IonideAddFileAbove", function(args)
                        M.AddFile(util.TrimParams(args.fargs[1]), M.TrimParams(args.fargs[2]),
                                M.TrimParams(args.fargs[3]),
                                "fsproj/addFileAbove")
                end,
        ...
        uc("IonideRenameFile", function(args)
                        M.RenameFile(util.TrimParams(args.fargs[1]), M.TrimParams(args.fargs[2]),
                                M.TrimParams(args.fargs[3]))
                end,
                { desc = "Renames an existing file" })

        uc("IonideRemoveCurrentFile", function()
                        M.RemoveCurrentFile()
                end,
                { desc = "Removes the current file" })

        uc("IonideRemoveFile", function(proj, file)
                        M.RemoveFileManually(proj, file)
                end,
                { desc = "Removes and existing file" })

        uc("IonideAddNewFileToProject", function(args)
                        M.AddNewFile(util.TrimParams(args.fargs[1]), M.TrimParams(args.fargs[2]))
                end,
                { desc = "Adds a new file to the project" })

        uc("IonideAddExistingFileToProject", function(args)
                        M.AddExistingFile(util.TrimParams(args.fargs[1]), M.TrimParams(args.fargs[2]))
                end,
